...
namespace IoTClient.gRPC
{
    internal class DataGenerator
    {
        ...
        public static EquipmentMessage GenerateData(int payloadSize)
        {
            ...
            message.Temperature = message.Status == "Running" ? rnd.Next(50, 99) : rnd.Next(10, 30);
        ...
        public static EquipmentMessage GenerateData_Tests(int payloadSize)
        {
            var message = new EquipmentMessage();
        ...
        private static IEnumerable<char> CreatePayload(int payloadSize)
        {
            ...
            var list = Enumerable.Repeat(0, numberofChars).Select(x => chars[random.Next(chars.Length-1)]);
            return list;
        }
    }
}
